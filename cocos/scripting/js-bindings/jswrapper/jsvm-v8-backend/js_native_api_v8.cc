...
#include "v8.h" //cjh added
//cjh #include "v8-debug.h"
#include "v8-internal.h"
//cjh #include "v8-local-handle.h"
...
//cjh  if (options && options->snapshotBlobData) {
...
//cjh v8::ScriptOrigin CreateScriptOrigin(
...
OH_JSVM_CompileScriptWithOrigin(JSVM_Env env,
  ...
  auto resourceName =
    ...
    v8::Local<v8::Boolean> resource_is_shared_cross_origin = v8::Boolean::New(isolate, false);
    v8::Local<v8::Integer> script_id = v8::Integer::New(isolate, -1);
    
    v8::Local<v8::Integer> resourceLineOffset = v8::Integer::New(isolate, origin->resourceLineOffset);
    v8::Local<v8::Integer> resourceColumnOffset = v8::Integer::New(isolate, origin->resourceColumnOffset);
    
  v8::ScriptOrigin scriptOrigin(resourceName,
    resourceLineOffset, resourceColumnOffset, resource_is_shared_cross_origin, script_id, sourceMapUrl);
 ...
 public:
  CompileOptionResolver(size_t length, JSVM_CompileOptions options[], v8::Isolate *isolate) {
    ...
    auto resourceName = v8::String::NewFromUtf8(isolate, sourceString.c_str()).ToLocalChecked();
      ...
      v8::Local<v8::Integer> resourceLineOffset = v8::Integer::New(isolate, jsvmOrigin ? jsvmOrigin->resourceLineOffset : 0);
      v8::Local<v8::Integer> resourceColumnOffset = v8::Integer::New(isolate, jsvmOrigin ? jsvmOrigin->resourceColumnOffset : 0);
      
    v8Origin = new v8::ScriptOrigin(resourceName,
      resourceLineOffset,
      resourceColumnOffset, resource_is_shared_cross_origin, script_id, sourceMapUrl);
...
//cjh  auto isolate = reinterpret_cast<v8::Isolate*>(vm);
...
//cjh  v8::Local<v8::Value> v8_script = v8impl::V8LocalValueFromJsValue(script);
//
//  RETURN_STATUS_IF_FALSE(env, v8_script->IsString(), JSVM_STRING_EXPECTED);
//
//  v8::ScriptCompiler::Source scriptSource(v8_script.As<v8::String>());
...
//cjh  auto backingStore = v8::ArrayBuffer::NewBackingStore(
...
JSVM_Status JSVM_CDECL OH_JSVM_GetTypedarrayInfo(JSVM_Env env,
  ...
  if (data != nullptr) {
//cjh    *data = static_cast<uint8_t*>(buffer->Data()) + array->ByteOffset();
...
JSVM_Status JSVM_CDECL OH_JSVM_IsDetachedArraybuffer(JSVM_Env env,
  ...
  *result =
//cjh      value->IsArrayBuffer() && value.As<v8::ArrayBuffer>()->WasDetached();
    value->IsArrayBuffer() && value.As<v8::ArrayBuffer>()->IsDetachable();
...
//cjh    maybe_module = v8::WasmModuleObject::Compile(env->isolate, {wasmBytecode, wasmBytecodeLength});
  } else {
      assert(false);//cjh
...
JSVM_Status JSVM_CDECL OH_JSVM_IsWasmModuleObject(JSVM_Env env,
  ...
  v8::Local<v8::Value> val = v8impl::V8LocalValueFromJsValue(value);
//cjh  *result = val->IsWasmModuleObject();
...
JSVM_Status JSVM_CDECL OH_JSVM_CreateWasmCache(JSVM_Env env,
  ...
  v8::Local<v8::Value> val = v8impl::V8LocalValueFromJsValue(wasmModule);
//cjh  RETURN_STATUS_IF_FALSE(env, val->IsWasmModuleObject(), JSVM_INVALID_ARG);
